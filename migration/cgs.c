...
bool cgs_mig_is_ready(bool is_src, const char *dst_ip, uint16_t dst_port)
{
    ...
    return cgs_mig.is_ready(is_src, dst_ip, dst_port);
}
...
int cgs_mig_savevm_state_abort(void)
{
    ...
    if (!cgs_mig.savevm_state_abort) {
        return 0;
    }

    ret = cgs_mig.savevm_state_abort();
    if (ret)
        error_report("%s: failed: %s", __func__, strerror(ret));
...
{
    info_report("calling cgs_mig_loadvm_create_tec");
    int ret;

    if (!cgs_mig.loadvm_create_tec) {
        return 0;
    }

    ret = cgs_mig.loadvm_create_tec(f);
    cgs_check_error(f, ret);
