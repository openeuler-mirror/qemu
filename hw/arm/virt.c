...
static const char *valid_cpus[] = {
    ...
    ARM_CPU_TYPE_NAME("Kunpeng-920-V1"),
    ARM_CPU_TYPE_NAME("Kunpeng-920-V2"),
    ARM_CPU_TYPE_NAME("Kunpeng-950-V1"),
                                      ...
                                      Error **errp)
{
    ...
    while (target_impl) {
        ...
        if (cnt >= MAX_TARGET_IMPL_CPUS) {
            error_setg(errp,
                       "Wrong format, Max %d targets can be set for now!", MAX_TARGET_IMPL_CPUS);
            return;
        }
