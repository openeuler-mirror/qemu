...
static void aarch64_a72_initfn(Object *obj)
{
    ...
    cpu->ctr = 0x8444c004;
    cpu->reset_sctlr = 0x00c50838;
    SET_IDREG(&cpu->isar, ID_PFR0, 0x00000131);
    SET_IDREG(&cpu->isar, ID_PFR1, 0x00011011);
    SET_IDREG(&cpu->isar, ID_DFR0, 0x03010066);
    cpu->id_afr0 = 0x00000000;
    SET_IDREG(&cpu->isar, ID_MMFR0, 0x10201105);
    SET_IDREG(&cpu->isar, ID_MMFR1, 0x40000000);
    SET_IDREG(&cpu->isar, ID_MMFR2, 0x01260000);
    SET_IDREG(&cpu->isar, ID_MMFR3, 0x02102211);
    SET_IDREG(&cpu->isar, ID_ISAR0, 0x02101110);
    SET_IDREG(&cpu->isar, ID_ISAR1, 0x13112111);
    SET_IDREG(&cpu->isar, ID_ISAR2, 0x21232042);
    SET_IDREG(&cpu->isar, ID_ISAR3, 0x01112131);
    SET_IDREG(&cpu->isar, ID_ISAR4, 0x00011142);
    SET_IDREG(&cpu->isar, ID_ISAR5, 0x00011121);
...
static void aarch64_kunpeng_920_v1_initfn(Object *obj)
{
    ARMCPU *cpu = ARM_CPU(obj);
    ...
    SET_IDREG(&cpu->isar, ID_AA64DFR0, 0x10305609);
    ...
    SET_IDREG(&cpu->isar, ID_AA64MMFR1, 0x110212122);
    ...
    SET_IDREG(&cpu->isar, ID_AA64ZFR0, 0x110100000100000);
...
static void aarch64_kunpeng_950_v1_initfn(Object *obj)
{
    ...
    SET_IDREG(&cpu->isar, ID_AA64DFR0, 0xf01030560a);
    SET_IDREG(&cpu->isar, ID_AA64DFR1, 0x0);

    SET_IDREG(&cpu->isar, ID_AA64ISAR0, 0x1221111110212120);
    SET_IDREG(&cpu->isar, ID_AA64ISAR1, 0x3011111100311002);

    SET_IDREG(&cpu->isar, ID_AA64MMFR0, 0x2100022200101025);
    SET_IDREG(&cpu->isar, ID_AA64MMFR1, 0x1010111110312124);
    SET_IDREG(&cpu->isar, ID_AA64MMFR2, 0x1221011110000011);

    SET_IDREG(&cpu->isar, ID_AA64PFR0, 0x1101001121110011);
    SET_IDREG(&cpu->isar, ID_AA64PFR1, 0x1000000021);
